...
class CNN(object):
    ...
    def cnn(self):
        x1,pw = self.gate1(self.input_ks,self.input_x1)
        new_x1 = self.addks(x1,pw)
        op1,op2 = self.conv(new_x1,self.input_x2)
        self.match(op1,op2)


    def gate1(self, ks, inputx):#pw = sigmoid([ks;e]*w*e)->[4,d]代表各个的概率,new_vector = pw[0] * ks[0] + pw[1] * ks[1] + pw[2] * ks[2] + pw[3] * e
    ...
    def addks(self,inputx,pw):
        inputx_epd = tf.expand_dims(inputx, axis=2)
        inputx_ks = tf.concat([inputx_epd, pw], axis=2)  # [None,l,5,d]
        inputx_ks_resh = tf.reshape(inputx_ks, shape=[-1, self.config.FACT_LEN,
                                                      (self.config.KS_LEN + 2) * self.config.EMBDDING_DIM])

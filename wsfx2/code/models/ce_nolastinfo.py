...
class CNN(object):
    ...
    def gate1(self, ks, inputx):
        with tf.name_scope("gate"):
            ...
            weight_2 = tf.Variable(tf.random_normal([self.config.EMBDDING_DIM, 1], #***************check!!!!
                                                    stddev=0, seed=2), trainable=True, name='w2')
            weight_3 = tf.Variable(tf.random_normal([self.config.EMBDDING_DIM, 1], #***************check!!!!
            ...
            ksw_2 = tf.sigmoid(tf.nn.relu(tf.einsum('abcd,abdf->abcf', fun2, k_2))) # [batch,l,d] #***************check!!!!

            fun3 = tf.einsum('abcd,de->abce', inputx_epd, weight_3)
            ksw_3 = tf.sigmoid(tf.nn.relu(tf.einsum('abcd,abdf->abcf', fun3, k_3)))  # [batch,l,d] #***************check!!!!

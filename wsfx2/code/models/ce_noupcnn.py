...
class CNN(object):
    ...
    def gate1(self, ks, inputx):
        with tf.name_scope("gate"):
            ...
            weight_2 = tf.Variable(tf.random_normal([self.config.EMBDDING_DIM, 2], #***************check!!!!
                                                    stddev=0, seed=2), trainable=True, name='w2')
            weight_3 = tf.Variable(tf.random_normal([self.config.EMBDDING_DIM, 2], #***************check!!!!
            ...
            ksw_1 = tf.sigmoid(tf.nn.relu(tf.einsum('abcd,abdf->abcf', fun1, k_1)))  #***************check!!!! # [batch,l,1,d]

            fun2 = tf.einsum('abcd,de->abce', inputx_epd, weight_2)
            ksw_2 = tf.sigmoid(tf.nn.relu(tf.einsum('abcd,abdf->abcf', fun2, tf.concat([k_2,ksw_1],axis=2)))) # [batch,l,d] #***************check!!!!

            fun3 = tf.einsum('abcd,de->abce', inputx_epd, weight_3)
            ksw_3 = tf.sigmoid(tf.nn.relu(tf.einsum('abcd,abdf->abcf', fun3, tf.concat([k_3,ksw_2],axis=2))))  # [batch,l,d] #***************check!!!!
    ...
    def match(self, op1):
        with tf.name_scope("match"):
            h = tf.concat(op1, axis=1)  # [batch,FILTERS*2] #***************check!!!!

...
class CNN(object):
    ...
    def gate1(self, ks, inputx):
        with tf.name_scope("gate"):
            ...
            ksw_1 = tf.sigmoid(tf.einsum('abcd,abdf->abcf', fun1, k_1))  # [batch,l,1,d] #***************check!!!!

            fun2 = tf.einsum('abcd,de->abce', inputx_epd, weight_2)
            ksw_2 = tf.sigmoid(tf.einsum('abcd,abdf->abcf', fun2, tf.concat([k_2,ksw_1],axis=2)) ) # [batch,l,d] #***************check!!!!

            fun3 = tf.einsum('abcd,de->abce',inputx_epd , weight_3)
            ksw_3 = tf.sigmoid(tf.einsum('abcd,abdf->abcf', fun3, tf.concat([k_3,ksw_2],axis=2))) # [batch,l,d] #***************check!!!!
